#define CFFI_MODULENAME "_jpg_lib_ffi"
...
static PyObject *
_cffi_f_write_jpeg(PyObject *self, PyObject *args)
{
  char const * x0;
  unsigned char * * x1;
  int x2;
  ...
  if (!PyArg_ParseTuple(args, "OOOOOO:write_jpeg", &arg0, &arg1, &arg2, &arg3, &arg4, &arg5))
    return NULL;
  ...
  _cffi_restore_errno();
  { result = write_jpeg(x0, x1, x2, x3, x4, x5); }
  _cffi_save_errno();
...
static PyMethodDef _cffi_methods[] = {
  {"write_jpeg", _cffi_f_write_jpeg, METH_VARARGS, NULL},
...
init_jpg_lib(void)
{
  PyObject *lib, *ffi;
  init_cffi_backend();
  ffi = PyImport_ImportModule("_jpg_lib_ffi");
  lib = Py_InitModule("_jpg_lib", _cffi_methods);
  if (lib == NULL)
    return;
  if (ffi == NULL || PyModule_AddObject(lib, "ffi", ffi) < 0)
    return;
